...
struct AgentExpandedContent: View {
  ...
  /// The FULL agent roster for the owning conversation instance (not just
  /// this agent's children), threaded into the embedded `Transcript`'s
  /// `allAgents` so `AgentDotResolver.isActive` can see live descendants
  /// beyond the visible/child set — e.g. a lead whose own dispatches
  /// finished but whose specialist is still working. Defaults to
  /// `childAgents` (the pre-fix behavior) when the caller has no broader
  /// roster to offer, matching the desktop AgentPanel's
  /// `allAgents = agents` default.
  ...
  /// Dispatch to pre-select when the pager first appears — a deep link
  /// (status-drawer entry) or a breadcrumb click into a specific historical
  /// dispatch, rather than defaulting to the most recent one. Resolved the
  /// same way `AgentDetailFullScreenView`'s own header resolves its subject
  /// (`AgentDotResolver.detailSubject`): an empty string or an id that no
  /// longer matches any dispatch falls through to "most recent", exactly
  /// like the no-initial-id case. Only the pinned/popup layout supplies
  /// this; AgentBarRow leaves it nil/empty because it has no deep-link entry
  /// point of its own.
    ...
    if pinHeader {
      ...
      VStack(alignment: .leading, spacing: 0) {
        ...
        Transcript(
          ...
          onRewind: nil,
          agents: childAgents ?? [],
          allAgents: allAgents ?? childAgents,
      ...
      .onAppear {
        // Seed the pager to the requested dispatch exactly once, on first
        // appear. Never re-seeds on a later appear (e.g. returning from a
        // pushed breadcrumb) so it cannot clobber the user's own pager
        // navigation. Mirrors AgentDotResolver.detailSubject's fallback: an
        // empty id or one that no longer matches any dispatch leaves
        // selectedDispatchIndex nil, which activeDispatch already resolves
        // to "most recent".
        guard selectedDispatchIndex == nil, !initialDispatchId.isEmpty else { return }
        if let idx = agent.dispatches.firstIndex(where: { $0.id == initialDispatchId }) {
  ...
  private func dispatchRow(for item: ConversationItem) -> some View {
    ...
    case .toolGroup(let tools):
      EngineToolGroupRow(tools: tools, activeBackgroundTasks: activeBackgroundTasks, tabId: tabId)
    ...
    case .agentTurn(let tools, let assistants, let isActive, let thinking):
      AgentTurnRow(
        tools: tools, assistantMessages: assistants, isActive: isActive, thinking: thinking,
        activeBackgroundTasks: activeBackgroundTasks, tabId: tabId
      )
      .padding(.horizontal, 10)  // design-geometry: 10pt gap between compactGap and contentGap; off the 4pt ratio scale
    case .system(let msg):
        ...
        ForEach(Array(agent.dispatches.enumerated().reversed()), id: \.element.id) { idx, d in
          ...
          Button {
            ...
            Text("#\(displayNum)")
              ...
              .background(
                isActive ? theme.surfaceElevated.opacity(0.7) : theme.surfaceElevated.opacity(0.3)
              )
  ...
  private func logDispatchState(event: String) {
    ...
    DiagnosticLog.log(
      "dispatch view", tag: "view.dispatch",
      fields: [
        "reason": event,
