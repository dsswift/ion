...
struct ConversationView: View {
    ...
    private var pendingPermission: PermissionRequest? {
        ...
        // Synthesize a card from conversation jesus Christ. Fabricant. Okay. Oh no, no no no no no no no. Okay in the Fabric Admin tenant settings Users can create fabric items is disabled for the entire organization. However capacity admins It does say however that this setting can be managed at the tenant and capacity levels. There doesn't seem to be anywhere that I can add individual users. users here Okay when I head over to the actual fabric capacity settings for my F2, I have the ability to turn it on and override the tenant section and specify my admin group. Morning What the fuck? Being able to create a league. is available again. Okay. new issue though when I click create new lake house I get the following error this is strange because we've already gone through the checklist here and I've already confirmed this is an f2 capacity on this workspace it is actually assigned to this workspace and that f2 capacity is enabled. I did save the capacity override it has not been 15 minutes worst case i'll wait and check again i did verify my account says pro license type The following are some examples of how you can use these features to create an environment for your project. Thank you for watching! Fuck. Fuck. Just got an email from Roberto. Keep in mind right now he's on the standard plan which only gives him access to sonnet so he physically can't use opus at work right now and there'll be a case-by-case strategic decision to release that model to developers that are more familiar with the tool and are doing more of the heavy lifting because of the enormous cost difference can be just as effective with actual task based work in the sonnet model make sure we paint it as a good thing not that he's missing out on anything Okay. Okay history when the permission queue
