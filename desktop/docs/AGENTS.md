...
Ion is a **macOS-only Electron overlay** that wraps the Claude Code CLI (`claude -p --output-format stream-json`) in a floating pill UI. It is NOT a web app, NOT a VS Code extension, and does NOT call the Anthropic API directly -- it spawns CLI subprocesses.
...
| Debug logging | `ION_DEBUG=1 npm run dev` (writes to `~/.ion-debug.log`) |
...
| **Preload** | `src/preload/` | Typed IPC bridge (`window.ion` API). Security boundary. |
...
```
InputBar.tsx → window.ion.prompt(tabId, requestId, opts)
...
4. Call from renderer via `window.ion.*`
