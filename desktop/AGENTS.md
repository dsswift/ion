...
- `node-pty` is legacy (still in dependencies for existing terminals). New subprocess work goes through `terminal-manager.ts` patterns. Note: `engine-bridge.ts` is **not** a subprocess spawner — the engine is a persistent supervised daemon (a launchd LaunchAgent on macOS, a per-user Scheduled Task on Windows) and the bridge only *connects* to its address (`main/engine-address.ts`); it never spawns the engine.
...
Target platforms are macOS and Windows (Studio). Every `process.platform === 'darwin'` guard needs a `win32` branch or a logged skip; a silent `return` off darwin is a defect. Use `path.delimiter` for PATH, `path.sep`-agnostic parsing (`shared/paths.ts`: `isAbsolutePath`, `pathSegments`, `joinPath`) for native paths, and `renderer/platform/mod-key.ts` (`isModKey`, `IS_MAC`) for the command modifier. No `/bin/*`, `/opt/*`, or `/usr/*` literal outside a darwin or linux branch. A test that pins darwin behavior pins win32 behavior for the same seam.
