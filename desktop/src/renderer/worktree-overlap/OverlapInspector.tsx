...
export function OverlapInspector({ pair }: { pair?: WorktreeOverlapPair }): React.JSX.Element {
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 12 }}>
...
function Section({ label, values, colors }: { label: string; values: string[]; colors: ReturnType<typeof useColors> }): React.JSX.Element | null {
  ...
  return <div><div style={{ color: colors.textTertiary, fontSize: 10, marginBottom: 3 }}>{label}</div>{values.map((value) => <div key={value} style={{ color: colors.textSecondary, fontFamily: 'monospace', fontSize: 11 }}>{value}</div>)}</div>
}
