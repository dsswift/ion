...
export function useEngineEvents() {
  ...
  useEffect(() => {
    ...
    const remoteUserMsgHandler = (_e: any, data: { tabId: string; requestId: string; prompt: string; timestamp: number; imageAttachments?: ImageAttachmentPayload[]; attachments?: Array<{ type: string; name: string; path: string; contentHash?: string }>; resolveSlash?: boolean; implementationPhase?: boolean; injectionKind?: string }) => {
      useSessionStore.getState().submitRemotePrompt(data.tabId, data.prompt, data.imageAttachments, data.resolveSlash, data.attachments, data.requestId, data.implementationPhase, data.injectionKind)
    }
    ...
    const remoteEnginePromptHandler = (_e: any, data: { tabId: string; text: string; reqId?: string; appendSystemPrompt?: string; imageAttachments?: ImageAttachmentPayload[]; attachments?: Array<{ type: string; name: string; path: string; contentHash?: string }>; resolveSlash?: boolean; implementationPhase?: boolean; injectionKind?: string }) => {
      useSessionStore.getState().submit(data.tabId, data.text, { appendSystemPrompt: data.appendSystemPrompt, imageAttachments: data.imageAttachments, remoteAttachments: data.attachments, source: 'remote', resolveSlash: data.resolveSlash, requestId: data.reqId, implementationPhase: data.implementationPhase, injectionKind: data.injectionKind })
    }
