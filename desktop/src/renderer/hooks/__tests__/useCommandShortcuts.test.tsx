...
describe("useCommandShortcuts", () => {
  ...
  it("uses Studio overrides without reading Overlay overrides", () => {
    ...
    // Mod is Cmd here: the test environment reports a macOS navigator.platform
    // (src/test/setup-globals.ts). The Mod-to-platform mapping itself is pinned
    // on both platforms by shortcuts/__tests__/chord.test.ts and
    // chord-win32.test.ts; this test is about which view's overrides are read.
    key("9", { metaKey: true });
