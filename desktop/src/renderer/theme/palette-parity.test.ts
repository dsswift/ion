...
describe("palette parity", () => {
  ...
  it("running stays distinct from the permission and error dots", () => {
    for (const [name, palette] of Object.entries({
      ...
      expect(p.statusRunning, `${name}: running vs permission`).not.toBe(
        p.statusPermission,
      );
      expect(p.statusRunning, `${name}: running vs error`).not.toBe(
        p.statusError,
      );
  ...
  it("shell dot is distinct from the error and question dots in every palette", () => {
    for (const [name, palette] of Object.entries({
      ...
      expect(
        p.statusBash,
        `${name}: statusBash must differ from statusError`,
      ).not.toBe(p.statusError);
      expect(
        p.statusBash,
        `${name}: statusBash must differ from statusDead`,
      ).not.toBe(p.statusDead);
      expect(
        p.statusBash,
        `${name}: statusBash must differ from statusQuestion`,
      ).not.toBe(p.statusQuestion);
      expect(
        p.statusBash,
        `${name}: statusBash must differ from statusPermission`,
      ).not.toBe(p.statusPermission);
      expect(
        p.statusBash,
  ...
  it("async dot is a distinct fuchsia, tuned per theme", () => {
    expect(darkColors.statusAsync).toBe("#e879f9");
    expect(hudColors.statusAsync).toBe("#e879f9");
    expect(lightColors.statusAsync).toBe("#c026d3");
    expect(classicColors.statusAsync).toBe("#d946ef");
    for (const [name, palette] of Object.entries({
      dark: darkColors,
      light: lightColors,
      classic: classicColors,
      ...
      expect(
        p.statusAsync,
        `${name}: statusAsync must differ from statusBash`,
      ).not.toBe(p.statusBash);
      expect(
        p.statusAsync,
        `${name}: statusAsync must differ from statusRunning`,
      ).not.toBe(p.statusRunning);
      expect(
        p.statusAsync,
        `${name}: statusAsync must differ from statusError`,
      ).not.toBe(p.statusError);
    }
  });
...
describe("Jarvis HUD freeze", () => {
  ...
  it("formerly-inherited HUD values stay pinned", () => {
    for (const [key, value] of Object.entries(frozenFormerlyInherited)) {
      expect(hudColors[key as keyof typeof hudColors], `hudColors.${key}`).toBe(
        value,
      );
    }
  });
...
describe("Ion Classic freeze", () => {
  ...
  it("classic matches the classic-era values pinned in the freeze map", () => {
    ...
    for (const [key, value] of Object.entries(frozenFormerlyInheritedRef)) {
      expect(
        classicColors[key as keyof typeof classicColors],
        `classicColors.${key}`,
      ).toBe(value);
    }
  });
