...
export function computeStatusCounts(
  ...
  for (const tab of tabs) {
    ...
    if (tab.status === "connecting") {
      c.connecting++;
      c.connectingTabs.push(ref(tab));
      continue;
    }
    if (anyEngineInstanceHasRunningChildren(tab.id)) {
      c.waitingChildren++;
      c.waitingTabs.push(ref(tab));
      continue;
    }
    ...
    if (permissionQueueLength > 0 || waitingState === "question") {
      c.questions++;
      c.questionTabs.push(ref(tab));
      continue;
    }
    if (waitingState === "plan-ready") {
      c.planReady++;
      c.planReadyTabs.push(ref(tab));
      continue;
    }
    if (tab.bashExecuting) {
      c.bash++;
      c.bashTabs.push(ref(tab));
      continue;
    }
    if (tabUnread(tab)) {
      c.unread++;
      c.unreadTabs.push(ref(tab));
      continue;
    }
    c.idle++;
    c.idleTabs.push(ref(tab));
  }
  return c;
}
    ...
    createPortal(
      ...
      >
        ...
        <WorkspaceCollapsibleRow
          ...
          expanded={expandedCategories.has("connecting")}
          label="Connecting"
          count={counts.connecting}
          color={colors.statusIdle}
          colors={colors}
          tabs={counts.connectingTabs}
          onToggle={handleToggleCategory}
          onNavigate={handleNavigate}
        />
        <WorkspaceCountRow
          label="Awaiting agents"
          count={counts.waitingChildren}
          color={colors.statusWaitingChildren}
          colors={colors}
        />
        ...
        <WorkspaceCountRow
          label="Awaiting shells"
          count={counts.waitingShells}
          color={colors.statusBash}
          colors={colors}
        />
        ...
        <WorkspaceCollapsibleRow
          categoryId="question"
          expanded={expandedCategories.has("question")}
          label="Question"
          count={counts.questions}
          color={colors.statusQuestion}
          colors={colors}
          tabs={counts.questionTabs}
          onToggle={handleToggleCategory}
          onNavigate={handleNavigate}
        />
        <WorkspaceCollapsibleRow
          categoryId="plan-ready"
          expanded={expandedCategories.has("plan-ready")}
          label="Awaiting plan"
          count={counts.planReady}
          color={colors.statusComplete}
          colors={colors}
          tabs={counts.planReadyTabs}
          onToggle={handleToggleCategory}
          onNavigate={handleNavigate}
        />
        <WorkspaceCollapsibleRow
          categoryId="bash"
          expanded={expandedCategories.has("bash")}
          label="Bash"
          count={counts.bash}
          color={colors.statusBash}
          colors={colors}
          tabs={counts.bashTabs}
          onToggle={handleToggleCategory}
          onNavigate={handleNavigate}
        />
        <WorkspaceCollapsibleRow
          categoryId="unread"
          expanded={expandedCategories.has("unread")}
          label="Unread"
          count={counts.unread}
          color={colors.statusComplete}
          colors={colors}
          tabs={counts.unreadTabs}
          onToggle={handleToggleCategory}
          onNavigate={handleNavigate}
        />
        <WorkspaceCollapsibleRow
          categoryId="idle"
          expanded={expandedCategories.has("idle")}
          label="Idle"
          count={counts.idle}
          color={colors.statusIdle}
          colors={colors}
          tabs={counts.idleTabs}
          onToggle={handleToggleCategory}
          onNavigate={handleNavigate}
        />
        <WorkspaceCollapsibleRow
          categoryId="dead"
          expanded={expandedCategories.has("dead")}
          label="Dead/failed"
          count={counts.dead}
          color={colors.statusError}
          colors={colors}
          tabs={counts.deadTabs}
          onToggle={handleToggleCategory}
          onNavigate={handleNavigate}
        />
      </div>,
        ...
        <span
          ...
          style={{
            ...
            ...(tier === "waiting"
              ...
              : {}),
            ...(tier === "running"
              ? { boxShadow: `0 0 5px 1px ${colors.statusRunning}40` }
              : {}),
          }}
        />
      </button>
