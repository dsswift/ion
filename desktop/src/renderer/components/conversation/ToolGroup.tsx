...
export const ToolGroup = React.memo(function ToolGroup({
  ...
  const hasExpandableTools =
    expandToolResults &&
    tools.some((t) => ["Edit", "Write"].includes(t.toolName || ""));
  const [expanded, setExpanded] = useState(
    hasUserExecuted || hasExpandableTools,
  );
        ...
        <div className="relative pl-6">
          ...
          <div className="space-y-1.5">
            {tools.map((tool) => {
              ...
              const asyncWaiting = running && !!tool.backgroundTaskId;
  ...
  const statusIcon = isRunning ? null : isAllFailed ? (
    <XCircle
      size={10}
  ...
  const inner = (
    ...
    >
      {isRunning ? (
        <SpinnerGap
          size={10}
          className="animate-spin flex-shrink-0 mt-[2px]"
          style={{ color: isRunning ? colors.statusRunning : colors.textSecondary }}
        />
      ) : (
        <CaretRight
          size={10}
      ...
      <span
        className="min-w-0 flex-1 flex items-center gap-1 text-[11px] leading-[1.4]"
        style={{
          color: isRunning ? colors.textSecondary : colors.textTertiary,
        }}
      >
