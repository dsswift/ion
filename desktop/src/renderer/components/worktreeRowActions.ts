 ...
 *
 * It is a plain factory, not a hook: it calls no React primitive. The menu
 * invokes it after all hook calls, preserving hook order on every render.
 *
...
export function buildWorktreeRowActions(
  deps: WorktreeRowActionsDeps,
): WorktreeRowActions {
  const {
    entry,
    repoPath,
