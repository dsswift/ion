...
import { ArrowsClockwise, ChatCircle, Trash, Warning } from '@phosphor-icons/react'
...
export function BenchVerificationDialog({
  ...
  const analyse = (): void => {
    ...
    void useSessionStore.getState()
      ...
      .then(() => onClose())
      .catch((err) => {
        rError('bench.verification', 'analysis open failed', { error: String(err) })
  ...
  const discardAndReassemble = (): void => {
    ...
    setBusy('discard')
    void useSessionStore.getState()
      .benchDiscardVerificationRecordings(repoPath, workspace.sourceBranch, suspects)
      .then((result) => {
        if (!result.ok) {
          rError('bench.verification', 'discard and reassemble failed', { error: result.error ?? '' })
      ...
      .catch((err) => {
        rError('bench.verification', 'discard and reassemble threw', { error: String(err) })
        setError(err instanceof Error ? err.message : String(err))
      })
      .finally(() => setBusy(null))
  }
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
        ...
        <div style={{ padding: '8px 10px', fontSize: 11, color: colors.textPrimary, lineHeight: 1.5 }}>
          Every member merged — including any replayed conflict resolutions — but the project&apos;s own
          verify command rejected the resulting tree. No conflict is outstanding; the bench was left
          empty exactly as a merge conflict would leave it, but there is nothing here to resolve by
          merging. This is not a conflict.
        </div>

        <div style={{ padding: '4px 10px 8px', fontSize: 10, color: colors.textSecondary }}>
          <div style={{ fontFamily: 'monospace', marginBottom: 4 }}>
            {evidence?.command || '(verify command unavailable)'}
          </div>
        </div>

        <div style={{ flex: 1, overflow: 'auto', minHeight: 0, padding: '0 10px' }}>
          <div style={{ fontSize: 10, color: colors.textTertiary, marginBottom: 4 }}>Output</div>
          <pre
            data-testid="bench-verification-output"
            style={{
              margin: 0, padding: 8, fontFamily: 'monospace', fontSize: 10,
              whiteSpace: 'pre-wrap', wordBreak: 'break-word',
              background: colors.surfaceHover, borderRadius: 4,
            ...
            <div style={{ marginTop: 10 }}>
              ...
              {suspects.map((branch) => (
                <div
                  ...
                  style={{ fontSize: 11, fontFamily: 'monospace', color: colors.textPrimary, padding: '2px 0' }}
                >
                  {branch}
                </div>
              ))}
        ...
        <div style={{ padding: '6px 10px', fontSize: 10, color: colors.textSecondary, lineHeight: 1.5 }}>
          Analyse opens a locked, read-only conversation that names whether this is a poisoned recording
          (discard and re-resolve) or a genuine incompatibility between members (fix it in the owning
          worktree, not here).
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderTop: `1px solid ${colors.containerBorder}` }}>
          ...
          >
            {busy === 'analyse' ? <ArrowsClockwise size={12} className="animate-spin" /> : <ChatCircle size={12} />}
            Analyse
          </button>
          <button
            ...
            style={{
              ...
              border: `1px solid ${colors.containerBorder}`, background: 'transparent',
              color: suspects.length === 0 ? colors.textTertiary : colors.textPrimary,
            }}
          >
            {busy === 'discard' ? <ArrowsClockwise size={12} className="animate-spin" /> : <Trash size={12} />}
      ...
      {confirmingDiscard && (
        <ConfirmDialog
          ...
          message={`Discard the recorded resolution${suspects.length === 1 ? '' : 's'} for ${suspects.join(', ')}? `
