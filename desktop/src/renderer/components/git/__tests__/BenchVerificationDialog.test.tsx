...
const benchDiscardVerificationRecordings = vi.fn(async () => ({ ok: true, forgottenCount: 1 }))

vi.mock('../../../stores/sessionStore', () => ({
  useSessionStore: Object.assign(
    (sel: (s: unknown) => unknown) => sel({ openBenchVerificationAnalysis, benchDiscardVerificationRecordings }),
    { getState: () => ({ openBenchVerificationAnalysis, benchDiscardVerificationRecordings }) },
  ),
}))
...
function failedWorkspace(over: Partial<IntegrationWorkspace> = {}): IntegrationWorkspace {
  return {
    repoPath: '/repo',
    sourceBranch: 'josh',
    benchPath: '/bench',
    ...
    lastAssemblyVerification: {
      ...
      outputTail: 'src/renderer/components/WorktreeRowMenu.tsx(122,8): error TS1109: Expression expected.',
...
function render(workspace: IntegrationWorkspace): void {
  act(() => {
    root.render(React.createElement(BenchVerificationDialog, {
      repoPath: '/repo',
      workspace,
      onClose,
    }))
...
describe('BenchVerificationDialog — renders the record alone, no IPC probe', () => {
  it('shows the exact verify command', () => {
    ...
    expect(host.textContent).toContain('cd engine && go build ./... && cd ../desktop && npm run typecheck')
  })

  it('shows the output tail', () => {
    render(failedWorkspace())
    expect(q('bench-verification-output')!.textContent).toContain('error TS1109')
  })
  ...
  it('disables the discard verb when there are no suspects to discard', () => {
    render(failedWorkspace({ lastAssemblyVerification: {
      command: 'exit 1', outputTail: '', replayedBranches: [],
    } }))
    expect((q('bench-verification-discard') as HTMLButtonElement).disabled).toBe(true)
  })
})
...
describe('BenchVerificationDialog — Dismiss', () => {
  it('closes without mutating anything', () => {
    render(failedWorkspace())
    act(() => { q('bench-verification-dismiss')!.click() })

    expect(onClose).toHaveBeenCalled()
    expect(openBenchVerificationAnalysis).not.toHaveBeenCalled()
    expect(benchDiscardVerificationRecordings).not.toHaveBeenCalled()
  })
})
...
describe('BenchVerificationDialog — Analyse', () => {
  it('opens the analysis conversation for this workspace and closes on success', async () => {
    render(failedWorkspace())
    act(() => { q('bench-verification-analyse')!.click() })
    await settle()
...
describe('BenchVerificationDialog — Discard recordings, gated by a count-bearing confirm', () => {
  it('shows a confirm naming the exact suspects before mutating anything', () => {
    render(failedWorkspace())
    act(() => { q('bench-verification-discard')!.click() })
  ...
  it('discards the named branches and closes only after confirming', async () => {
    ...
    const confirmBtn = buttons.find((b) => b.textContent?.startsWith('Discard 2'))
    expect(confirmBtn).toBeTruthy()

    act(() => { confirmBtn!.click() })
    await settle()

    expect(benchDiscardVerificationRecordings).toHaveBeenCalledWith('/repo', 'josh', ['wt/a', 'wt/c'])
    expect(onClose).toHaveBeenCalled()
  })
})
