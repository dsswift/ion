...
export function AgentDetailPanel({
  ...
  const [subMessages, setSubMessages] = useState<Map<string, Message[]>>(
    new Map(),
  );
  ...
  const childTelemetry =
    dispatchTelemetry && top.dispatchId
  ...
  const childAgentStates: AgentStateUpdate[] = [
    ...childAgentPills,
    ...telemetryOnlyStubs,
  ];
  ...
  const handleOpenDispatch = useCallback(
    (dispatch: DispatchInfo, childAgent: AgentStateUpdate) => {
  ...
  const rowDispatches = isRoot
    ? dispatches
    : (() => {
        const owner = (allAgents ?? []).find((candidate) =>
          getDispatches(candidate).some(
            (dispatch) => dispatch.id === top.dispatchId,
          ),
        );
        return owner ? getDispatches(owner) : topDispatch ? [topDispatch] : [];
      })();
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        >
          ...
          {tabId &&
            topDispatch &&
            topStatus === "running" &&
            runningRowDispatchIds.includes(topDispatch.id) && (
              <div
                style={{
                  position: "absolute",
                  bottom: 4,
              ...
              >
                <DispatchStopControl
                  dispatchId={topDispatch.id}
                  runningDispatchIds={runningRowDispatchIds}
