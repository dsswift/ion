  ...
  return (
    <Reorder.Item
      ...
      onMouseDown={(e) => {
        ...
        if (e.button === 1) {
          e.preventDefault()
          // Worktree tabs included: close is non-destructive since the
          // lifecycle split, so excluding them only removed the affordance.
    ...
    >
      ...
      {/* Confirmation is the shared CloseTabConfirmDialog (raised by
          requestCloseTab through onCloseTab), not an inline Yes/No: this row is
          as narrow as the pill was and could not carry the worktree warning. */}
      {!isRunning && (
        <button
