...
interface State {
  ...
  // Engine state (keyed by compound key tabId:instanceId)
...
export const useSessionStore = create<State>((set, get) => ({
  ...
  closeTab: (tabId) => {
    ...
    // Clean up engine state (compound keys start with tabId:)
    ...
    if (profile) {
      window.ion.engineStart(key, {
        ...
        if (result && !result.ok) {
          ...
          // Show persistent error in conversation + notification
          set((state) => {
            ...
            // Add error as a system message so it persists in the conversation
            ...
            // Reset tab status to idle so it doesn't stick on "Thinking..."
  ...
  removeEngineInstance: (tabId, instanceId) => {
    ...
    set({ engineMessages, engineAgentStates, engineStatusFields, engineWorkingMessages, engineNotifications, engineDialogs, enginePinnedPrompt, engineUsage })
  },
    ...
    switch (event.type) {
      ...
      case 'engine_text_delta': {
        set((state) => {
          ...
          messages.set(key, msgs)
          // Set tab running only if this is the active instance
