...
describe("applyResourceSnapshot — ID normalization", () => {
  ...
  it("uses final duplicate read state", () => {
    const result = applyResourceSnapshot(
      ...
      "briefing",
      "sub-1",
      [makeItem("a", "stale-read", true), makeItem("a", "final-unread", false)],
    );
    expect(result.resources.briefing).toEqual([
  ...
  it("keeps read state separate when kinds share an ID", () => {
    const state = applyResourceDelta(initialResourceState, "briefing", {
      op: "mark_read",
      item: { ...makeItem("shared"), producer: "alpha" },
    });
    ...
    expect(
      result.readResourceIds.has(
        resourceIdentity({ id: "shared", producer: "alpha", kind: "briefing" }),
      ),
    ).toBe(true);
    expect(
      result.readResourceIds.has(
        resourceIdentity({ id: "shared", producer: "beta", kind: "alert" }),
      ),
    ).toBe(false);
  });
  ...
  it("migrates a legacy raw-ID read key to every producer sharing that ID", () => {
    ...
    expect(result.readResourceIds).toEqual(
      new Set(["8:briefing:5:alpha:same", "8:briefing:4:beta:same"]),
    );
  });

  it("preserves a read identity when a later snapshot says unread", () => {
    ...
    const result = applyResourceSnapshot(
      { ...initialResourceState, readResourceIds: new Set([identity]) },
      "briefing",
      "sub-1",
      [{ ...makeItem("read-on-device"), producer: "alpha", read: false }],
      ["alpha"],
    );

    expect(result.readResourceIds.has(identity)).toBe(true);
    expect(result.readResourceIds.has("read-on-device")).toBe(false);
  });
...
describe("applyResourceSnapshot — producer identity", () => {
  it("keeps same-ID items from different producers", () => {
    ...
    expect(result.resources.briefing.map((item) => item.content)).toEqual([
      "from-a",
      "from-b",
    ]);
  });
