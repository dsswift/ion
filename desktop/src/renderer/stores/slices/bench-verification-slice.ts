...
export function createBenchVerificationSlice(
  set: StoreSet,
  get: StoreGet,
): Partial<State> {
  return {
    openBenchVerificationAnalysis: async (repoPath, sourceBranch) => {
      const ws = (get().benchWorkspaces.get(repoPath) ?? []).find(
        (w) => w.sourceBranch === sourceBranch,
      );
      if (!ws) {
      ...
      const tier = await resolveWorkbenchTier({
        workflow: "bench-verification-analysis",
        repoPath,
        sourceBranch,
      });
      ...
      rInfo("bench.verification", "preparing diagnostic tree", {
        repo_path: repoPath,
        source_branch: sourceBranch,
      });
      const prepared = await window.ion.benchPrepareVerificationAnalysis(
        repoPath,
        sourceBranch,
      );
      if (!prepared.ok || !prepared.benchPath) {
        ...
        throw new Error(
          prepared.error ??
            "Could not rebuild the failing bench tree for analysis.",
        );
      }
      ...
      const refreshed = (get().benchWorkspaces.get(repoPath) ?? []).find(
        (w) => w.sourceBranch === sourceBranch,
      );
      const evidence = refreshed?.lastAssemblyVerification;
      const replayedMembers = (evidence?.replayedBranches ?? []).map(
        (branchName) => ({
          branchName,
          worktreePath:
            (refreshed ?? ws).members.find((m) => m.branchName === branchName)
      ...
      const rendered = renderAiAssistTemplate(
        workflowId,
        template,
        verificationAnalysisValues({
          sourceBranch,
          verifyCommand:
            evidence?.command ?? "(unknown — evidence unavailable)",
      ...
      set((s) => ({
        tabs: s.tabs.map((t) =>
          t.id === tabId
            ? {
                ...t,
                tabRole: "verification-analysis" as const,
                inputLocked: true,
