...
type PromptModelSelection =
  | Pick<
      ...
      "modelOverride" | "modelOverrideSource" | "sessionModel"
    >
  | null
  | undefined;
...
function resolvePromptThinkingEffort(
  ...
  const explicitSlashModel =
    isSlashPrompt && instance?.modelOverrideSource === "user";
  const effortModelId =
    isSlashPrompt && !explicitSlashModel
      ? undefined
      : instance?.modelOverride || instance?.sessionModel || preferredModel;
  const effortModel = effortModelId
    ? useModelStore.getState().findModel(effortModelId)
    : undefined;
  return effortModel
    ? resolveEffortForModel(
        rawEffort,
        effortModel.thinkingMode,
        effortModel.thinkingEfforts ?? [],
      )
    : rawEffort;
}
  ...
  return {
    ...
    applySendAutoGroupMove: (tabId) => {
      ...
      if (cancelDoneGroupMove(tabId)) {
        rDebug("auto-move.send", "cancelled pending done-move", {
    ...
    interrupt: (tabId, scope = "all") => {
      ...
      const hasRunningChildren = (inst?.agentStates ?? []).some(
        (a) => a.status === "running",
      );
      const backgroundTaskCount =
        inst?.statusFields?.activeBackgroundTasks?.length ?? 0;
      rInfo("interrupt", "interrupt requested", {
        tab_id: tabId.slice(0, 8),
        status: tab.status,
        abort_scope: scope,
        has_running_children: hasRunningChildren,
        background_task_count: backgroundTaskCount,
        bash_exec_id: tab.bashExecId ?? "",
      });
    ...
    submit: (tabId, text, opts = {}) => {
      ...
      if (!tab) {
        ...
        rWarn("submit", "refused: no such tab", {
          tab_id: tabId.slice(0, 8),
          count: text.length,
        });
      ...
      set((s) => {
        ...
        const userMessage = {
          id:
            source === "remote" && isSteer && clientRequestId
              ? clientRequestId
              : nextMsgId(),
          role: "user" as const,
        ...
        const conversationPanes = commitInstance(
          ...
          (inst) => ({
            ...
            messages: suppressBubble
              ? inst.messages
              : [...inst.messages, userMessage],
      ...
      if (tab.forkedFromSessionId && !tab.conversationId) {
        ...
        if (priorMessages.length > 0) {
          ...
          effectiveSystemPrompt = effectiveSystemPrompt
            ...
            : forkCtx;
          rDebug("submit", "rewind context injected", {
            tab_id: tabId.slice(0, 8),
      ...
      window.ion.setPermissionMode(
        ...
        syncMode,
        "prompt_sync",
        syncMode === "plan" ? sendInst?.planFilePath || undefined : undefined,
      );

      let extensions: string[] | undefined;
      if (tab.engineProfileId) {
        const profile = usePreferencesStore
          .getState()
          .engineProfiles.find((p) => p.id === tab.engineProfileId);
      ...
      const thinkingEffort = resolvePromptThinkingEffort(
        isSlashPrompt,
        sendInst,
        rawEffort,
        preferredModel,
    ...
    submitRemotePrompt: (
      ...
      set((s) => {
        ...
        steerClientMessageId =
          isBusy && !isImplementation ? userMessage.id : undefined;
        const conversationPanes = commitInstance(
          s.conversationPanes,
          tabId,
          (inst) => ({
            ...inst,
            messages: suppressRemoteBubble
              ? inst.messages
              : [...inst.messages, userMessage],
      ...
      window.ion.setPermissionMode(
        ...
        syncMode,
        "prompt_sync",
        syncMode === "plan" ? remoteInst?.planFilePath || undefined : undefined,
      );
      ...
      const remoteRawEffort = effectiveThinkingEffort(
        tab,
        get().conversationPanes,
      );
