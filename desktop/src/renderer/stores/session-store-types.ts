// @file-size-exception: merged store contract retained during rebase
/**
...
export interface State extends WorktreeBenchActions, EngineSubmitActions {
  ...
  fileExplorerStates: Map<
    string,
    { expandedPaths: Set<string>; selectedPath: string | null }
  >;
  ...
  workspaceOperationLedger: Map<
    string,
    import("./session-store-worktree-sync").WorkspaceOperation
  >;

  engineWorkingMessages: Map<string, string>;
  engineNotifications: Map<
    string,
    Array<{ id: string; message: string; level: string; timestamp: number }>
  >;
  engineDialogs: Map<
    string,
    {
  ...
  closeTab: (
    tabId: string,
    origin?: "local" | "remote" | "delete" | "remote-delete",
  ) => void;
  /**
  ...
  reorderPinnedTabs: (
    assignments: ReadonlyArray<{ id: string; orderKey: string }>,
  ) => void;
