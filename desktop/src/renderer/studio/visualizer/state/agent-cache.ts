...
export class AgentCache {
  ...
  private adopt(tabId: string, snapshot: StudioTabState, profileId: string | null): void {
    ...
    if (changed) {
      ...
      this.recorder.clear()
      rInfo('studio', 'retarget', { tab_id: tabId, agent_count: snapshot.agents.length })
    ...
    switch (event.type) {
      case 'agent_state': {
        ...
        if (this.lastStatusSig.get(tabId) !== sig) {
          ...
          rInfo('studio', 'agent statuses changed', { tab_id: tabId, statuses: sig })
        } else {
          rTrace('studio', 'agent snapshot heartbeat', { tab_id: tabId, count: event.agents.length })
        }
