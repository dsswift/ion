 ...
 *
 * I/O and decoding are injected (StudioAssetSource / decode option) so node
...
export interface LoadedCharacter {
  manifest: StudioCharacterManifest
  animations: Record<string, LoadedStrip>
}

export interface LoadedPet {
  manifest: StudioPetManifest
  animations: Record<string, LoadedStrip>
}
...
export interface LoadedTheme {
  theme: StudioThemeManifest
  tileSize: number
  characters: Map<string, LoadedCharacter>
  pets: Map<string, LoadedPet>
  furniture: Map<string, LoadedFurniture>
  floors: Map<string, { manifest: StudioFloorManifest; bitmap: StudioBitmap }>
  walls: Map<string, { manifest: StudioWallManifest; bitmap: StudioBitmap }>
  bubbles: Record<StudioBubbleKind, StudioBitmap>
  dressing: Map<string, StudioDressingTemplate>
...
export interface StudioAssetSource {
  listThemes(): Promise<StudioThemeListEntry[]>
  readBundle(packId: string): Promise<StudioRawPackBundle | null>
...
export interface LoadThemeOptions {
  ...
  decode?: (bytes: ArrayBuffer) => Promise<StudioBitmap>
  /** Sink for per-asset load logs. Defaults to silent (tests) — the Studio shell passes the renderer logger. */
...
async function defaultDecode(bytes: ArrayBuffer): Promise<StudioBitmap> {
  return createImageBitmap(new Blob([bytes], { type: 'image/png' }))
}

/** Validate one raw bundle into a ValidatedPack, collecting skip reasons. */
export function validateBundle(bundle: StudioRawPackBundle, skipped: string[]): ValidatedPack | null {
...
export async function loadTheme(
  ...
  if (!pack) {
    for (const s of skipped) warn('studio theme skip', { detail: s })
    throw new Error(`Studio theme "${packId}" failed to load: ${skipped.join(' | ') || 'unknown error'}`)
  }
  ...
  if (failures.length > 0) {
    throw new Error(`Studio theme "${packId}" unusable: ${failures.join('; ')}`)
  }
  return loaded
}
