 ...
 * fixtures. Contract: docs/design/visualizer/theme-pack-format.md.
 */
import type {
  StudioAnimationSpec,
  StudioBubblesManifest,
  StudioCharacterManifest,
  StudioDressingTemplate,
  StudioFloorManifest,
...
function checkAnimations(
  ...
  errors: string[],
): Record<string, StudioAnimationSpec> {
  const out: Record<string, StudioAnimationSpec> = {}
  if (!isRecord(v)) {
...
export function validateThemeManifest(json: unknown): ValidationResult<StudioThemeManifest> {
...
export function validateCharacterManifest(json: unknown): ValidationResult<StudioCharacterManifest> {
  ...
  return {
    ...
    value: {
      id: json.id as string,
      name: json.name as string,
      roles: json.roles as StudioCharacterManifest['roles'],
...
export function validatePetManifest(json: unknown): ValidationResult<StudioPetManifest> {
...
export function validateFurnitureManifest(json: unknown): ValidationResult<StudioFurnitureManifest> {
  ...
  if (errors.length > 0) return fail(errors)
  return { ok: true, value: json as unknown as StudioFurnitureManifest }
}

export function validateFloorManifest(json: unknown): ValidationResult<StudioFloorManifest> {
  ...
  if (errors.length > 0) return fail(errors)
  return { ok: true, value: json as unknown as StudioFloorManifest }
}

export function validateWallManifest(json: unknown): ValidationResult<StudioWallManifest> {
  ...
  checkFile(json.file, errors, 'file')
  if (errors.length > 0) return fail(errors)
  return { ok: true, value: json as unknown as StudioWallManifest }
}

export function validateBubblesManifest(json: unknown): ValidationResult<StudioBubblesManifest> {
  ...
  if (errors.length > 0) return fail(errors)
  return { ok: true, value: json as unknown as StudioBubblesManifest }
}

export function validateDressingTemplate(json: unknown, expectedZone: string): ValidationResult<StudioDressingTemplate> {
  ...
  if (errors.length > 0) return fail(errors)
  return { ok: true, value: json as unknown as StudioDressingTemplate }
}
...
export function expectedAnimationDims(spec: StudioAnimationSpec, tileSize: number): ExpectedDims {
...
export function expectedFurnitureDims(
  manifest: StudioFurnitureManifest,
  key: string,
  tileSize: number,
): ExpectedDims {
