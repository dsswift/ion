...
/** An in-memory StudioAssetSource over fixture packs. */
export function memorySource(packs: FixturePack[]): StudioAssetSource {
  const byId = new Map(packs.map((p) => [p.bundle.packId, p]))
  return {
    listThemes: async (): Promise<StudioThemeListEntry[]> =>
      packs.map((p) => ({
        id: p.bundle.packId,
