/**
 * useExports — the postcard (PNG) and clip (WebM) export callbacks,
 * extracted from VisualizerRoot to keep it under the file-size cap. Pure composition
...
export interface ExportDeps {
  ...
  activeRef: React.MutableRefObject<StudioActiveState | null>
...
export function useExports({ canvasRef, activeRef, cacheRef, seed, clip, setClip }: ExportDeps): {
  ...
  const recordClip = useCallback(() => {
    ...
    try {
      ...
      rError('studio', 'clip recorder unavailable', { error: String(err) })
      return
    }
