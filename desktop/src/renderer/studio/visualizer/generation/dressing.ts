...
function manifest(theme: GenTheme, id: string): StudioFurnitureManifest | undefined {
  return theme.furniture.get(id)
}

function defaultVariant(m: StudioFurnitureManifest): string {
...
function footprintTiles(m: StudioFurnitureManifest, x: number, y: number): Array<{ x: number; y: number }> {
...
function isFree(ctx: DressContext, room: Room, m: StudioFurnitureManifest, x: number, y: number, seats: Seat[]): boolean {
...
function place(ctx: DressContext, room: Room, m: StudioFurnitureManifest, x: number, y: number, opts?: { onSurface?: boolean; variant?: string }): PlacedFurniture {
...
function perSeatItems(ctx: DressContext, template: StudioDressingTemplate): {
  desk: StudioFurnitureManifest | null
  chair: StudioFurnitureManifest | null
  onDesk: StudioFurnitureManifest | null
} {
  const perSeat = template.required
    .filter((e) => e.perSeat && e.id)
    .map((e) => manifest(ctx.theme, e.id!))
    .filter((m): m is StudioFurnitureManifest => !!m)
  return {
...
function dressClusterRoom(ctx: DressContext, room: Room, spec: RoomSpec, template: StudioDressingTemplate): void {
...
function dressCountRoom(ctx: DressContext, room: Room, template: StudioDressingTemplate, seatKind: 'manager' | 'head' | null): void {
  const ix = room.interior.x
  // Same one-row circulation margin as cluster rooms (door-wall clearance).
  const iy = room.interior.y + 1
  let surface: { m: StudioFurnitureManifest; x: number; y: number } | null = null
...
function placeOnWall(ctx: DressContext, room: Room, m: StudioFurnitureManifest): boolean {
...
function dressOptional(ctx: DressContext, room: Room, template: StudioDressingTemplate): void {
  ...
  const candidates = template.optional
    ...
    .filter((c): c is { entry: (typeof template.optional)[0]; m: StudioFurnitureManifest } => !!c.m)
...
function pickByCategory(ctx: DressContext, category?: string): StudioFurnitureManifest | undefined {
...
function pickWallItem(ctx: DressContext, category?: string): StudioFurnitureManifest | undefined {
  const pool = [...ctx.theme.furniture.values()]
