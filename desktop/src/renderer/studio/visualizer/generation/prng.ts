/**
 * Seeded PRNG for all Studio randomness (generation, casting, ambient behavior).
 *
 * Determinism contract: same seed string always yields the same stream, on
 * every platform. Nothing in Studio generation may use Math.random or Date-based
 * entropy — every random choice flows through an StudioRng created here.
 */
...
export function createRng(seed: string): StudioRng {
