 ...
 */
import type { StudioRng } from '../generation/prng'
...
const CONFETTI_COLORS = ['#ff5f5f', '#ffd23c', '#3ecf6e', '#5fa8ff', '#c95fff', '#ff9d3c'] // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
export const MAX_PARTICLES = 160

/** Burst of confetti around (x, y) in tile coordinates. */
export function spawnConfetti(rng: StudioRng, x: number, y: number, n: number): Particle[] {
  const out: Particle[] = []
  for (let i = 0; i < n; i++) {
    out.push({
      x: x + (rng.next() - 0.5) * 2,
