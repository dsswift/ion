...
export function heatColor(count: number, p95: number): string | null {
  ...
  return `rgba(${r}, ${g}, ${b}, ${alpha.toFixed(3)})` // hardcoded-ok: runtime canvas color derived from Studio scene palette
}
...
export class SceneFx {
  ...
  visitP95 = 1
  /** Heat overlay toggle (toolbar-driven; persisted as studioHeat). */
  heatEnabled = false
