/**
 * createVisualizerEngine — wires generation, simulation, and rendering behind a
...
export interface StudioEngine {
  ...
  /**
   * Pause/resume the render+simulation loop (surface-tab deactivation).
   * Event ingestion continues upstream (AgentCache) so a resume is
   * instant; only the rAF loop halts — display:none stops paint but not
   * the loop, so an inactive visualizer tab would otherwise keep burning
   * frames.
   */
  pause(): void
  resume(): void
...
export function createVisualizerEngine(canvas: HTMLCanvasElement, theme: LoadedTheme): StudioEngine {
  ...
  return {
    ...
    resume() {
      // ensureLoop() created the loop on first scene; a paused loop just
      // restarts its rAF chain. No-op when the loop never existed.
      loop?.start()
    },
    destroy() {
      loop?.stop()
