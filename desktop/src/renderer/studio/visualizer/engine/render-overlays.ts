/**
 * render-overlays — the Studio window's post-scene draw passes (badges today;
...
export function drawLighting(ctx: CanvasRenderingContext2D, layout: OfficeLayout, fx: SceneFx, tile: number): void {
  ...
  for (const room of layout.rooms) {
    ...
    if (b >= 0.98) continue
    ctx.fillStyle = `rgba(10, 12, 24, ${(1 - b) * 0.75})` // hardcoded-ok: runtime canvas color derived from Studio scene palette
  ...
  if (fx.globalBrightness < 0.98) {
    ctx.fillStyle = `rgba(8, 10, 22, ${(1 - fx.globalBrightness) * 0.5})` // hardcoded-ok: runtime canvas color derived from Studio scene palette
...
function drawGlyph(ctx: CanvasRenderingContext2D, kind: BadgeKind, x: number, y: number): void {
  ctx.strokeStyle = '#e8e8ec' // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
  ctx.fillStyle = '#e8e8ec' // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
  ctx.lineWidth = 1
  switch (kind) {
    case 'terminal': // >_
      ctx.beginPath()
...
export function drawDashboards(
  ...
  for (const placed of office.layout.furniture) {
    ...
    if (kind === 'kanban') {
      ...
      if (pts.length > 1) {
        ctx.strokeStyle = '#3ecf6e' // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
        ctx.lineWidth = 1
        ctx.beginPath()
...
export function drawFxSprites(ctx: CanvasRenderingContext2D, office: OfficeState, fx: SceneFx, tile: number, animClock: number): void {
  ...
  for (const entity of office.entities.values()) {
    ...
    ctx.fillStyle = 'rgba(90, 95, 110, 0.9)' // hardcoded-ok: runtime canvas color derived from Studio scene palette
    ...
    ctx.fillStyle = 'rgba(120, 160, 220, 0.85)' // hardcoded-ok: runtime canvas color derived from Studio scene palette
