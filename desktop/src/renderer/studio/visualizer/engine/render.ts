...
const TINT_TONES = ['#566073', '#6f7a90'] // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
...
export function renderOffice(
  ...
  ctx.fillStyle = '#14161c' // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
  ...
  for (const entity of office.entities.values()) {
    ...
    let alpha = 1
    if (entity.working) {
      color = '#ff8c3c' // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
      alpha = pulse
    } else if (entity.waiting) {
      color = '#ffd23c' // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
      alpha = pulse
    } else if (entity.completed) {
      color = '#3ecf6e' // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
    }
    if (!color) continue
    ctx.save()
