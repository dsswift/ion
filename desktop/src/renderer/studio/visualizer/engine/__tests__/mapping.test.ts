...
describe("snapshot diffing", () => {
  ...
  it("a newly running NESTED agent is delivered by its owning lead", () => {
    ...
    const specialist = agent("backend-dev", "running", {
      dispatchDepth: 2,
      dispatchParentId: "d1",
    });
  ...
  it("a nested agent with an unknown owner falls back to the manager", () => {
    const orphan = agent("backend-dev", "running", {
      dispatchDepth: 2,
      dispatchParentId: "d-gone",
    });
  ...
  it("initial scene builds place working agents WITHOUT replaying deliveries", () => {
    expect(
      diffSnapshots([], [agent("dev", "running")], { initial: true }),
    ).toEqual([{ kind: "agent-working", agent: "dev" }]);
  });
