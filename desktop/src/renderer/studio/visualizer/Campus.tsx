/**
 * Campus — the zoomed-out campus: one building per conversation, glowing
...
export function Campus(props: CampusProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const draw = (): void => {
      ...
      ctx.fillStyle = '#101408' // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
      ctx.fillRect(0, 0, canvas.width, canvas.height)
      ctx.save()
      ctx.translate(ox, oy)
      ctx.scale(zoom, zoom)
      // Lawn.
      ctx.fillStyle = '#1c2a14' // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
      ...
      for (const b of buildings) {
        ...
        ctx.restore()
        // Body + roof.
        ctx.fillStyle = '#2c2f3a' // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
        ctx.fillRect(px, py, pw, ph)
        ctx.fillStyle = '#3a3e4c' // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
        ctx.fillRect(px - 2, py - 6, pw + 4, 8)
        // Lit windows = working agents (capped by facade space).
        const lit = Math.min(entry.working, 8)
        for (let w = 0; w < 8; w++) {
          ctx.fillStyle = w < lit ? '#ffd76a' : '#191c24' // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
          ctx.fillRect(px + 6 + (w % 4) * 14, py + 8 + Math.floor(w / 4) * 16, 8, 10)
        }
        // Label.
        ctx.fillStyle = '#c8ccd8' // hardcoded-ok: Studio canvas palette is theme-pack-driven, not app-theme-driven
