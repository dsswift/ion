...
vi.mock('../../stores/sessionStore', () => ({
  useSessionStore: Object.assign(
    (selector: (value: typeof state) => unknown) => useSyncExternalStore(
      (listener) => {
...
describe('InboxNavigatorGroups external selection', () => {
  ...
  it('shows whichever conversation becomes active while its group stays collapsed', async () => {
    ...
    expect(container.querySelector('[data-testid="conversation-older"]')).toBeNull()
    ...
    await act(async () => { activateOutsideInbox('older') })

    expect(container.querySelector('[data-testid="conversation-older"]')).not.toBeNull()
    expect(container.querySelector('[data-testid="conversation-newer"]')).toBeNull()
    expect(container.querySelector('[aria-label="Toggle Source Repository"]')).not.toBeNull()

    await act(async () => { activateOutsideInbox('newer') })
