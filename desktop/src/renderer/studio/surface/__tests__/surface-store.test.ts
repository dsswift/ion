...
describe("surface-store", () => {
  it("keeps each conversation active tab when both have local selections", () => {
    ...
    expect(useSurfaceStore.getState().tabs.map((tab) => tab.id)).toEqual([
      "plan",
    ]);
  ...
  it("restores each conversation active tab even when the other conversation uses a global pin", () => {
    ...
    expect(useSurfaceStore.getState().activeTabId).toBe("dispatch-preview");
    expect(
      useSurfaceStore.getState().tabs.filter((tab) => tab.kind === "dispatch"),
    ).toHaveLength(1);
  });
  ...
  it("reuses the conversation dispatch preview tab for a later dispatch click", () => {
    ...
    const dispatchTabs = useSurfaceStore
      .getState()
      .tabs.filter((tab) => tab.kind === "dispatch");
  ...
  it("opens and focuses a workspace notification over the active Plan tab", () => {
    ...
    const state = useSurfaceStore.getState();
    expect(state.tabs.find((tab) => tab.id === "notification")).toMatchObject({
  ...
  it("keeps a workspace notification open across conversations until closed", () => {
    ...
    store.openResourceTab({ id: "resource-1", kind: "briefing" } as never);
    expect(
      useSurfaceStore.getState().tabs.find((tab) => tab.id === "notification"),
    ...
    expect(
      useSurfaceStore.getState().tabs.find((tab) => tab.id === "notification"),
    ).toMatchObject({ resourceId: "resource-2" });
    store.selectConversation("tab-1");
    expect(
      useSurfaceStore.getState().tabs.find((tab) => tab.id === "notification"),
    ).toMatchObject({ resourceId: "resource-2" });
    ...
    store.selectConversation("tab-2");
    expect(
      useSurfaceStore.getState().tabs.some((tab) => tab.id === "notification"),
    ).toBe(false);
  });
  ...
  it("closes the panel when closing the last tab empties the conversation strip", () => {
    const store = useSurfaceStore.getState();
    ...
    expect(state.conversations["tab-1"]?.visible).toBe(false);
  });

  it("leaves the panel open when a tab close still leaves tabs behind", () => {
    const store = useSurfaceStore.getState();
    store.openSingleton("diff");
    store.setVisible(true);
    store.closeTab("diff");
    const state = useSurfaceStore.getState();
    expect(state.tabs.map((tab) => tab.id)).toEqual(["plan"]);
  ...
  it("materializes a file buffer for the owning conversation", () => {
    ...
    expect(
      useSurfaceStore
        .getState()
        .tabs.some((tab) => tab.id === "file:/repo/a.ts"),
    ).toBe(true);
  });
  ...
  it("creates browse tabs with a shared browser session by default", () => {
    ...
    expect(
      useSurfaceStore.getState().tabs.find((tab) => tab.kind === "browser"),
    ).toMatchObject({
      mode: "browse",
      sessionMode: "shared",
    });
  ...
  it("destroys a terminal only when explicitly closed", () => {
    ...
    const terminal = useSurfaceStore
      .getState()
    ...
    expect(terminalDestroyMock).toHaveBeenCalledWith(
      `tab-1:surface:${(terminal as { instanceId: string }).instanceId}`,
    );
  });
