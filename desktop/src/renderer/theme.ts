...
window.ion?.loadSettings().then((disk) => {
  ...
  const engineExtensionPath = typeof disk.engineExtensionPath === 'string' ? disk.engineExtensionPath : '~/.pi/extensions/chief-of-staff.ts'
  ...
  if (engineProfiles.length === 0 && engineExtensionPath) {
    ...
    useThemeStore.getState().addEngineProfile(migrated)
  }
})
