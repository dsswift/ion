  ...
  switch (event.type) {
    ...
    case "engine_image_content":
      ...
      ctx.emit("event", tabId, {
        ...
        source: event.imageSource,
        ...(event.imageContentHash
          ? { contentHash: event.imageContentHash }
          : {}),
    ...
    case "engine_user_turn_persisted":
      ...
      ctx.emit("event", tabId, {
        ...
        slashFrontmatter: event.userTurnSlashFrontmatter,
      } as NormalizedEvent);
      void import("./automation/runtime")
        .then(({ getAutomationRuntime }) =>
          getAutomationRuntime().triggerResolvedSlash(
            tabId,
            event.userTurnSlashFrontmatter,
          ),
        )
        .catch((err) =>
          error("resolved slash automation trigger failed", {
            tab_id: tabId,
            error: String(err),
          }),
        );
    ...
    case "engine_export":
      ...
      log("export", {
        tab_id: tabId,
        format: event.exportFormat ?? "absent",
        payload_bytes: event.message?.length ?? 0,
      });
