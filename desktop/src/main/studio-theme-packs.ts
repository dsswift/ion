/**
 * studio-theme-packs — main-process discovery and raw reads for Studio theme packs.
 *
 * Discovery, not enumeration: the pools are whatever pack directories exist.
 * Two roots are scanned:
 *   - bundled: <resources>/studio/themes  (ships with the app)
 *   - user:    ~/.ion/studio/themes      (user-installed packs)
...
function bundledRoot(): string {
  return join(__dirname, '../../resources/studio/themes')
}

function userRoot(): string {
  return join(homedir(), '.ion', 'studio', 'themes')
}
