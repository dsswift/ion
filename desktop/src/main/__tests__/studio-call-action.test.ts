/**
 * studio:call-action — the mirror→owner round trip in the main process.
 *
 * The Studio window runs the session store in MIRROR mode, so owner-durable
...
vi.mock('../studio-window-manager', () => ({
  openStudioWindow: vi.fn(), applyStudioActivationPolicy: vi.fn(), isStudioWindowOpen: vi.fn(() => false),
}))
vi.mock('../window-manager', () => ({ showWindow: vi.fn() }))
vi.mock('../studio-state-cache', () => ({ getStudioState: vi.fn(() => null), allStudioSummaries: vi.fn(() => []) }))
...
function replyAs(senderId: number, callId: string, value: unknown): void {
  ...
  for (const fn of onListeners.get(IPC.STUDIO_ACTION_RESULT) ?? []) fn(event, callId, value)
}
...
function callAction(action: unknown, args: unknown): Promise<{ ok: boolean; value?: unknown; error?: string }> {
  const handler = handlers.get(IPC.STUDIO_CALL_ACTION)
  if (!handler) throw new Error('STUDIO_CALL_ACTION handler not registered')
  return handler({}, action, args) as Promise<{ ok: boolean; value?: unknown; error?: string }>
}

describe('studio:call-action round trip', () => {
  beforeEach(() => {
    handlers.clear()
    onListeners.clear()
    sent.length = 0
  ...
  it("relays a validated action and resolves the owner's return value", async () => {
    ...
    const relayed = sent.find((s) => s.channel === IPC.STUDIO_EXEC_ACTION)
  ...
  it('ignores a reply carrying a different callId', async () => {
    ...
    const callId = (sent.find((s) => s.channel === IPC.STUDIO_EXEC_ACTION)!.args[2] as string)
  ...
  it('rejects an action outside FORWARDED_ACTIONS without relaying', async () => {
    // handleNormalizedEvent is MIRROR_LOCAL — forwarding it would let the Studio window
    ...
    expect(sent.filter((s) => s.channel === IPC.STUDIO_EXEC_ACTION)).toHaveLength(0)
  })
  ...
  it('leaves no reply listener behind on either path', async () => {
    vi.useFakeTimers()
    const before = (onListeners.get(IPC.STUDIO_ACTION_RESULT) ?? []).length
    ...
    await replied
    expect((onListeners.get(IPC.STUDIO_ACTION_RESULT) ?? []).length).toBe(before)
   ...
   * not settle the call.
   *
   * STUDIO_ACTION_RESULT is an ipcMain.on listener, so any renderer holding the
   * preload bridge can send on it, and the callId is a predictable counter
   * (`studio-call-N`). Without the sender check a non-owner window could hand the
  ...
  it('logs a reply that arrives after the timeout', async () => {
    ...
    // The module's log() wrapper calls _log('studio', msg, fields), so the message
