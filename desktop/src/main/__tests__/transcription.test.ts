 ...
 *
 * Related spec: specs/issue-ion-3-async-execfile-transcription.tests.md
 */
...
describe('TC-003: WhisperKit fallback runs when first run produces no stdout', () => {
  ...
  it('calls whisper binary twice when first run returns empty stdout and no report JSON', async () => {
    ...
    const tmpWav = '/tmp/ion-voice-test.wav'
    const reportDir = '/tmp'
    ...
    // Check report file
    const wavBasename = 'ion-voice-test'
  ...
  it('returns correct result shape from handler', async () => {
    ...
    const reportExists = mockExistsSync(join('/tmp', 'ion-voice-test.json'))
