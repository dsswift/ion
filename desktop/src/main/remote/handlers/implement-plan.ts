...
export async function handleImplementPlan(
  ...
  try {
    ...
    log("handle_implement_plan: plan file lookup failed", {
  ...
  try {
    const escapedTab = tabId.replace(/\\/g, "\\\\").replace(/'/g, "\\'");
    const modelGroupResult = (await state.mainWindow?.webContents
      .executeJavaScript(`
      (function() {
        try {
          var store = window.__Ion_SESSION_STORE__;
    ...
    if (modelGroupResult && !modelGroupResult.ok) {
      log("handle_implement_plan: renderer model/group rejected", {
        reason: modelGroupResult.reason ?? "unknown",
      });
    }
  } catch (err) {
    log("handle_implement_plan: renderer model/group step failed", {
      error: (err as Error).message,
    });
  }
  ...
  try {
    ...
    const slugSuffix = planSlug
      ? ` \\u00b7 ${planSlug.replace(/\\/g, "\\\\").replace(/'/g, "\\'")}`
      : "";
    const planPathJs = planFilePath
      ? `'${planFilePath.replace(/\\/g, "\\\\").replace(/'/g, "\\'")}'`
      : "null";
    const mutationResult = (await state.mainWindow?.webContents
      .executeJavaScript(`
      (function() {
        try {
          var store = window.__Ion_SESSION_STORE__;
    ...
    if (mutationResult && !mutationResult.ok) {
      log("handle_implement_plan: renderer state mutation rejected", {
        reason: mutationResult.reason ?? "unknown",
      });
    }
  } catch (err) {
    log("handle_implement_plan: renderer state mutation failed", {
      error: (err as Error).message,
    });
  }
  ...
  try {
    ...
    if (tabInfo) {
      ...
      if (!resolvedInstanceId)
        resolvedInstanceId = tabInfo.activeInstanceId || null;
      worktree = tabInfo.worktree || null;
    }
  } catch (err) {
    // Probe failure degrades to defaults (no extensions, caller-supplied
    // instanceId); log so the degraded routing is diagnosable.
    debug("implement_plan: tab-info renderer probe failed", {
      tab_id: tabId,
      error: String(err),
    });
  }
