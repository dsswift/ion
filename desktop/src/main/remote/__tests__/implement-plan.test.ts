...
vi.mock("../../state", () => ({
  ...
  sessionPlane: {
    ...
    resolvePermissionDenials: vi.fn(),
  },
  engineBridge: {},
...
const mockTriggerPlanImplemented = vi.fn().mockResolvedValue(undefined);
...
beforeEach(() => {
  ...
  mockTriggerPlanImplemented.mockClear();
...
describe("handleImplementPlan", () => {
  ...
  it("continues dispatch after a renderer script rejection", async () => {
    ...
    await handleImplementPlan({
      type: "desktop_implement_plan",
      tabId: "tab-script-rejection",
      questionId: "q-script-rejection",
    });
  ...
  it("triggers plan implementation automation from remote command", async () => {
    await handleImplementPlan({
      type: "desktop_implement_plan",
      tabId: "tab-remote",
      questionId: "q-remote",
    ...
    expect(mockTriggerPlanImplemented).toHaveBeenCalledWith(
      "tab-remote",
