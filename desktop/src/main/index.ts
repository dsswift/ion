...
const DEBUG_MODE = process.env.Ion_DEBUG === '1'
const SPACES_DEBUG = DEBUG_MODE || process.env.Ion_SPACES_DEBUG === '1'
...
engineBridge.on('event', (key: string, event: any) => {
  ...
  if (remoteTransport) {
    ...
    remoteTransport.send({ type: `engine_${event.type.replace('engine_', '')}`, tabId, instanceId, ...event })
  }
})
...
/**
 * Extract Ion-prepended bash command results from a user message.
