...
import { applyAtvActivationPolicy } from './atv-window-manager'
...
export function openWorktreeOverlapWindow(next: WorktreeOverlapContext): void {
  ...
  if (existing && !existing.isDestroyed()) {
    existing.show()
    existing.focus()
    log('focused existing window', { repo_path: next.repoPath, source_branch: next.sourceBranch ?? '' })
    return
  }
  ...
  win.webContents.on('render-process-gone', (_event, details) => {
    _error(TAG, 'renderer process gone', { reason: details.reason, exit_code: details.exitCode })
  })
  ...
  if (process.env.ELECTRON_RENDERER_URL) {
    void win.loadURL(`${process.env.ELECTRON_RENDERER_URL}/worktree-overlap.html`).catch((error) => {
      _error(TAG, 'could not load development renderer', { error: String(error) })
    })
  } else {
    void win.loadFile(join(__dirname, '../renderer/worktree-overlap.html')).catch((error) => {
      _error(TAG, 'could not load packaged renderer', { error: String(error) })
    })
  }
  log('created window', { repo_path: next.repoPath, source_branch: next.sourceBranch ?? '' })
}
...
export function worktreeOverlapContext(senderId: number): WorktreeOverlapContext | null {
  const win = state.worktreeOverlapWindow
