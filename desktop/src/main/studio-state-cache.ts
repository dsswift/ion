/**
 * studio-state-cache — main-process per-tab cache backing the Agent Team
 * Visualizer window.
 *
 * The Studio window can open at any time, including long after a tab's agent
 * team started working. Events flow through broadcast() whether or not the
 * Studio window exists, so this cache taps that stream and keeps, per tab:
 *
 ...
 *   - a capped ring of raw dispatch/permission events (the Studio window renderer
 ...
 *
 * The Studio renderer pulls this via `studio:get-state` on window open and on every
...
/**
 * Normalized-event types the Studio window consumes. Everything else on the
...
export interface StudioTabState {
   ...
   * on permission_request; removed on answer (resolveStudioPermission, fired
