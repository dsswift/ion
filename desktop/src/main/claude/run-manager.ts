...
const DEBUG = process.env.Ion_DEBUG === '1'
...
// safety. This is a deliberate Ion policy choice, not native Claude parity.
...
export class RunManager extends EventEmitter {
  ...
  startRun(requestId: string, options: RunOptions): RunHandle {
    ...
    if (options.hookSettingsPath) {
      // Ion-scoped hook settings: the PreToolUse HTTP hook handles permissions
    ...
    // Always tell Claude it's inside Ion (additive, doesn't replace base prompt)
    const appendParts = [Ion_SYSTEM_HINT]
