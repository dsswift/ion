...
describe("schedule serialization (post-init RPC path)", () => {
  it('forwards concurrency="all" for daily/weekly/interval when requested (regression #226)', async () => {
    const cases: Array<{
      kind: string;
      register: (mod: any) => Promise<unknown>;
    }> = [
      {
        kind: "daily",
        register: (mod) =>
          mod.scheduleApi.daily({
            id: "d1",
            time: "09:00",
            concurrency: "all",
  ...
  it('forwards catchUp="manual" for daily and weekly schedules', async () => {
    for (const register of [
      ...
      markInitResolved(mod);
      await register(mod);
      expect(calls[0].params.catchUp).toBe("manual");
    }
  });
...
describe("fire_async dispatch round-trips", () => {
  ...
  it("returns {ok:true} for a schedule fire", async () => {
    ...
    const out = (await mod.dispatchFireAsync(
      { kind: "schedule", id: "fire-sched", payload: {} },
      buildContext,
    )) as any;
  ...
  it("throws on unknown kind and unknown id", async () => {
    ...
    await expect(
      mod.dispatchFireAsync(
        { kind: "mystery", id: "x", payload: {} },
        buildContext,
      ),
    ).rejects.toThrow();
    await expect(
      mod.dispatchFireAsync(
        { kind: "schedule", id: "missing", payload: {} },
        buildContext,
      ),
    ).rejects.toThrow();
  });
...
describe("resolve_token / resolve_predicate dispatch", () => {
  it("resolves a registered token and fails closed to empty when absent", async () => {
    ...
    const hit = (await mod.dispatchResolveToken({
      name: "webhook:/tok-hook:token",
    })) as any;
    expect(hit.value).toBe("resolved-secret");
    const miss = (await mod.dispatchResolveToken({
      name: "webhook:/nope:token",
    })) as any;
  ...
  it("resolves a registered predicate and fails OPEN to enabled when absent", async () => {
    ...
    const hit = (await mod.dispatchResolvePredicate({
      name: "schedule:pred-sched:enabled",
    })) as any;
    expect(hit.enabled).toBe(false);
    const miss = (await mod.dispatchResolvePredicate({
      name: "schedule:absent:enabled",
    })) as any;
...
describe("unregister", () => {
  it("post-init webhook unregister issues ext/deregister_webhook and stops dispatch", async () => {
    const { mod, calls } = await freshRuntime();
    markInitResolved(mod);
    const handle = await mod.webhooksApi.register({
      path: "/u-hook",
      auth: { kind: "none" },
      handler: noopWebhookHandler,
    });
    ...
    await expect(
      mod.dispatchFireAsync(
        { kind: "webhook", id: "/u-hook", payload: {} },
        (raw: any) => raw,
      ),
    ).rejects.toThrow();
  });
  ...
  it("post-init schedule unregister issues ext/deregister_schedule", async () => {
    const { mod, calls } = await freshRuntime();
    markInitResolved(mod);
    const handle = await mod.scheduleApi.daily({
      id: "u-sched",
      time: "00:00",
      handler: noopScheduleHandler,
    });
