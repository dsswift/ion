...
Core: Read, Write, Edit, Bash, Poll, Grep, Glob, Agent, AgentStatus, WebFetch, WebSearch, NotebookEdit, LSP, Skill, ListMcpResources, ReadMcpResource, SearchHistory, WorktreeList, WorktreeCommits, WorktreeDiff, TodoWrite.
...
While developing, follow the root [`AGENTS.md`](../AGENTS.md) § "Validation cadence — never after every file edit". Run a narrow test after a logical batch when it can disprove the changed behavior. Run the required scoped gate set once when the implementation is stable. Do not rerun a successful gate unless later code changes could invalidate it. Do **not** run the full `go test -race ./...` sweep, integration tests, or `govulncheck` mid-development; those are heavy gates that run at PR time — CI is authoritative, and `/create-pr` runs the Linux parity subset once before pushing.
