...
**Background bash completion** is the third async delivery path alongside schedules and webhooks. A `Bash` call with `run_in_background: true, notify_on_complete: true` joins the session's outstanding set; when the command exits, the engine emits `engine_background_task_complete`, fires the `background_task_completed` hook, and (under the default `wake` delivery mode) starts a run carrying the result. Unlike schedules and webhooks it is not registered by an extension — the model opts in per tool call — but it shares their shape: work completes outside any turn, and the engine routes it back into the session. The root session parks at the turn boundary while commands are outstanding and is woken once per completion. See [ADR-023](../docs/architecture/adr/023-root-session-park-and-wake.md) and [`docs/tools/task-tools.md`](../docs/tools/task-tools.md) § "Background bash completion".
