...
// the given model. For non-hybrid backends (plain ClaudeCodeBackend or ApiBackend,
...
func (m *Manager) resolvedBackend(model string) backend.RunBackend {
	...
	inner := h.ResolveFor(model)
	utils.LogWithFields(utils.LevelDebug, "session", "resolvedBackend routed via hybrid", map[string]any{"model": model})
	return inner
}
...
// instead of calling NewApiBackend()/NewClaudeCodeBackend() directly, so that
...
func (m *Manager) newChildBackend() backend.RunBackend {
	...
	case *backend.ClaudeCodeBackend:
		return backend.NewClaudeCodeBackend()
...
// InnerApi), which also implements it. ClaudeCodeBackend and test stubs do not, so
...
// (InnerApi), which also implements it. ClaudeCodeBackend and test stubs do not, so the
