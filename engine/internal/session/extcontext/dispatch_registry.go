...
// RecalledDispatch identifies a recalled registry entry for durable lifecycle updates.
type RecalledDispatch struct {
	DispatchID string
	SessionID  string
	Name       string
}
...
// SetDispatchLossRecallObserver receives every target and descendant before cancellation.
// It lets the session manager persist recall intent across conversation files.
func (r *DispatchRegistry) SetDispatchLossRecallObserver(observer func([]RecalledDispatch)) {
	r.mu.Lock()
	defer r.mu.Unlock()
