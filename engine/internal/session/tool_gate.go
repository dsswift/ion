...
// wireClientTools adds the session's client-declared tools
// (ToolGateConfig.ClientTools) to the run's tool list and chains their
// fulfillment into the external-tool router. Client tools are the third tool
...
func (m *Manager) wireClientTools(s *engineSession, key string, runCfg *backend.RunConfig) {
	gateCfg := s.config.ToolGate
	...
	clientToolNames := make(map[string]bool, len(gateCfg.ClientTools))
	for _, ct := range gateCfg.ClientTools {
		...
		// ClientToolDef is field-identical to LlmToolDef (modulo JSON tags),
		// so this is a direct type conversion, per staticcheck S1016.
		runCfg.ExternalTools = append(runCfg.ExternalTools, types.LlmToolDef(ct))
	...
	runCfg.McpToolRouter = func(ctx context.Context, name string, input map[string]interface{}) (*types.ToolResult, error) {
		if clientToolNames[name] {
			return m.requestClientToolResult(ctx, key, captured, name, input, s.config.WorkingDirectory), nil
		}
		if priorRouter != nil {
	...
	utils.LogWithFields(utils.LevelInfo, "session.toolgate", "client tools wired", map[string]any{
		"key": key, "count": len(clientToolNames), "timeout_ms": captured.ResolveClientToolTimeoutMs(),
	})
}
...
// the fulfillment timeout elapses, or the tool context is cancelled. Always
// returns a non-nil ToolResult: a missing client is a tool error the model
// can read, never a hang.
