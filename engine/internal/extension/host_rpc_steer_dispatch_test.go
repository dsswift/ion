...
// steerDispatchByNamePayload builds the JSON-RPC frame for ext/steer_dispatch_by_name.
func steerDispatchByNamePayload(t *testing.T, name, message string) []byte {
	t.Helper()
	data, err := json.Marshal(map[string]interface{}{
		"jsonrpc": "2.0",
		"id":      1,
		"method":  "ext/steer_dispatch_by_name",
		"params": map[string]interface{}{
			"name":    name,
			"message": message,
		},
...
// TestExtSteerDispatchByName_Delivered verifies the happy path for
// ext/steer_dispatch_by_name: ctx.SteerDispatchByName is wired, the handler
// calls it with the agent name, and the response carries delivered=true.
func TestExtSteerDispatchByName_Delivered(t *testing.T) {
	h := NewHost()
	ch := attachStdout(h)
	...
	h.handleExtRequest("ext/steer_dispatch_by_name", 1, steerDispatchByNamePayload(t, "code-reviewer", "focus on error handling"))
	...
	if gotName != "code-reviewer" {
		t.Errorf("name passed = %q, want %q", gotName, "code-reviewer")
	}
	if gotMsg != "focus on error handling" {
		t.Errorf("message passed = %q, want %q", gotMsg, "focus on error handling")
	}
}

// TestExtSteerDispatchByName_NotFound verifies that a not_found outcome from
// the name-based steer (no matching dispatch) is forwarded correctly.
func TestExtSteerDispatchByName_NotFound(t *testing.T) {
	h := NewHost()
	ch := attachStdout(h)
	...
	h.handleExtRequest("ext/steer_dispatch_by_name", 1, steerDispatchByNamePayload(t, "nonexistent-agent", "hi"))
...
// TestExtSteerDispatchByName_NotAvailable verifies the handler returns an error
// when ctx.SteerDispatchByName is nil (no registry wired).
func TestExtSteerDispatchByName_NotAvailable(t *testing.T) {
	h := NewHost()
	ch := attachStdout(h)
	...
	h.handleExtRequest("ext/steer_dispatch_by_name", 1, steerDispatchByNamePayload(t, "any-agent", "msg"))
	...
	msg, _ := errObj["message"].(string)
	if msg != "steer dispatch by name not available" {
		t.Errorf("error message = %q, want 'steer dispatch by name not available'", msg)
	}
}
