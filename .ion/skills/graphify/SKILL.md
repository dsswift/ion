...
> **Ion note — read this before any step below.**
>
> **Query the existing graph. Do not probe for it, do not rebuild it, and do not
> install anything.**
>
> ```bash
...
> ```
>
> That is the whole procedure for a codebase question in this repo. Run it as
> your first action. If it fails — graphify absent, no graph in this clone — the
> failure costs ~2ms and is your signal to answer with `Grep`/`Glob`/`Read`
> instead. Fall through silently; a missing graph is not a setup error.
>
> **Do not run Step 1 below.** It offers to *install* graphify when missing.
> Installing a tool into a contributor's environment because a repo skill
> mentions it is not a decision this skill gets to make. Ion treats graphify as
> optional for contributors: nothing in the project requires it, and a clone
...
## For the commit hook and native CLAUDE.md/AGENTS.md integration

When the user asks to install the post-commit auto-rebuild hook or wire graphify into a project's CLAUDE.md or AGENTS.md, see `references/hooks.md`.
