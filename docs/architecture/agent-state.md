...
- **Protected keys are never removed.** Protected values are never shortened or marker-replaced: `displayName`, type, visibility, invitation state, status, color, dispatch IDs, conversation IDs, and `dispatches[]` retain their original types and values. This ensures a client can still address each dispatch and retrieve its durable content.
