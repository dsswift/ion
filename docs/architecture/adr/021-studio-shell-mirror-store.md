...
Ptys are owned by the main process with server-side scrollback (`terminalScrollback`, accumulated unconditionally in `broadcast()`), lifecycle records that outlive the pty (exit retains buffer + exit code), and an attach protocol (`TERMINAL_ATTACH`: history snapshot + live stream, respawn-on-demand, dead-cwd fallback to `~` with a visible notice). Conversation Terminal Panel metadata is owner-controlled and mirrored between Overlay and Studio through a complete revisioned snapshot, so both presentations show the same terminal IDs, labels, order, selection, and panel visibility while attaching to the same PTYs. Renderers are viewers: unmount detaches, and only explicit close destroys. Studio Surface terminals remain separate Studio-only descriptors in the `<conversationId>:surface:<instanceId>` key namespace; `ion://terminal` never targets them. Surface terminals persist across app restarts via `main/studio-terminal-persistence.ts`; conversation terminals keep riding `PersistedTab.terminalBuffers`. Terminals therefore survive window closes, live mode switches, and app restarts.

## Parity enforcement (automatic, not aspirational)

1. **Reuse is the parity system.** A surface shared by both presentations is ONE component reading the same store. Bespoke Studio widgets are permitted only for canvas-coupled surfaces (marquee, inspector, control bar).
...
3. **Broadcast parity gate.** `make check-studio-parity` (CI: quality.yml) fails any direct `webContents.send` in `desktop/src/main` outside the documented owner-only allowlist — new event pushes reach both presentations by default.
