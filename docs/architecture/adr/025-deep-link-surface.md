...
Two properties of the existing desktop made this tractable. Terminal PTYs are owned by the Electron **main** process (`main/terminal-manager.ts`, keyed `"<tabId>:<instanceId>"`) with each renderer's xterm acting as a viewer, so a pane can be created and stream output while its tab has never been mounted — the iOS `desktop_terminal_add_instance` path already did this. Conversation Terminal Panel metadata is owner-controlled and mirrored between Overlay and Studio, so the same terminal tabs attach to those PTYs in either presentation. Studio Surface terminals are separate `<conversationId>:surface:<instanceId>` sessions and are never deep-link targets. And `getCliEnv(extra)` already accepted an environment overlay, so per-PTY identity had somewhere to live.
