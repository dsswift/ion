...
Before emission, the engine persists a `pending` delivery marker for the dispatch. On every later engine restart, the event and hook are re-emitted until the consumer acknowledges durable handling through `ctx.ackDispatchLost(dispatchId)` (or the `ext/ack_dispatch_lost` RPC). Acknowledgement changes the marker to `sent` and suppresses future delivery for that dispatch ID. The event is still not replayed merely because a client reconnects; restart rehydration is the delivery boundary.
