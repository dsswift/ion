...
| `appContext`                | object (string map) | no | Per-prompt refresh of the session's application-context descriptor; see `EngineConfig.appContext` above for the full semantics. A non-nil map here replaces the session's stored value for the rest of the session; absent leaves it unchanged. |
