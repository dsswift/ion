...
Every worktree outside the selected cohort names a concrete reason: a merge counterpart and its conflict paths, no committed contribution, a missing comparison base, or work already landed in the source branch. Large sets use an explicitly labelled anchored search rather than claiming a global optimum. Auto-order never factorial-searches more than the bounded candidate count; larger selections retain their current order after exact merge validation. Selecting a cohort changes only visualizer state until Apply confirms its exact membership and merge-order changes.
...
The visualizer lets the operator select an eligible clean cohort, review exact membership changes, then atomically persist the exact bench member set and merge order. It never assembles a bench or advances an existing pin. Apply revalidates every selected path, rejects duplicates, and refuses persistence failures.
